<%@ Page Title="" Language="C#" MasterPageFile="~/Teacher.Master" AutoEventWireup="true" CodeBehind="TeacherAddQuizMarks.aspx.cs" Inherits="SchoolPRO.TeacherAddQuizMarks" %>
                ...
                <asp:MultiView ID="mvmarks" ActiveViewIndex="0" runat="server">
                    <asp:View ID="View1" runat="server">
                        <div class="table-responsive">
                            ...
                            <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:SchoolPro %>" SelectCommand="SELECT distinct c.strClass,c.nc_id, sc.strSection,sc.nsc_id,s.strCourseCode, s.strSubject,s.nsbj_id FROM tbl_TimeTable AS t INNER JOIN tbl_Class AS c ON t.nc_id = c.nc_id inner join tbl_Section sc on t.nsc_id=sc.nsc_id INNER JOIN tbl_Subject AS s ON t.nsbj_id = s.nsbj_id INNER JOIN tbl_Users AS u ON t.nu_id = u.nu_id WHERE (u.strEmail =@uem ) and t.bisDeleted='False' and t.nsch_id=@schid ">
                                <SelectParameters>
                    ...
                    <asp:View ID="View2" runat="server">
                        <div class="table-responsive">
                            ...
                            <asp:DropDownList ID="ddexams" runat="server" AppendDataBoundItems="true" OnSelectedIndexChanged="ddexams_SelectedIndexChanged" AutoPostBack="true" DataSourceID="examtypDS" DataTextField="strExamName" DataValueField="nExam_id">
                                <asp:ListItem Text="<---Select Type of Quiz--->" Value="--"></asp:ListItem>



                            </asp:DropDownList>

                            <asp:SqlDataSource runat="server" ID="examtypDS" ConnectionString='<%$ ConnectionStrings:SchoolPro %>' SelectCommand="SELECT nExam_id, strExamName, bisDeleted, nsch_id FROM tbl_Quiz WHERE (bisDeleted = @bit) AND (nsch_id = @schid)">
                                <SelectParameters>
                            ...
                            <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:SchoolPro %>" SelectCommand="SELECT ne_id,strStudentNum, strFname, strLname FROM tbl_Enrollment WHERE (nc_id =  @cnm) and nsc_id=@sc  AND (strStudentNum IS NOT NULL) and nsch_id=@sn and bisDeleted='False'">
                                <SelectParameters>
