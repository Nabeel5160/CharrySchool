...
namespace SchoolPRO
{
    public partial class AdminManageP_C : System.Web.UI.Page
    {
       
        protected void Page_Load(object sender, EventArgs e)
        {
            //dddonr.Visible = false;
            //if (ddgaurd.Text == "Donor")
            //{
            //    dddonr.Visible = true;
            //}
        ...
        void GenrerateStudentNumber()
        {
            ...
            cmd.CommandText = "select strStudentNum from  tbl_Enrollment where ne_id=(select Max(ne_id) as Id from tbl_Enrollment where  nsc_id=('" + ddsec.SelectedItem.Value + "') and nc_id=('"+ddcl.SelectedItem.Value+"') and bisDeleted='False' and nsch_id= '" + Session["nschoolid"] + "')";
          //  cmd.CommandText = "select strStudentNum from  tbl_StudentNumber where ne_id=(select Max(ne_id) as Id from tbl_StudentNumber where  nsc_id=(select nsc_id from tbl_Section where strSection='" + ddsec.Text + "' and nc_id=(select nc_id from tbl_Class where strClass='" + ddcl.Text + "' and bisDeleted='False')and bisDeleted='False') and nc_id=(select nc_id from tbl_Class where strClass='" + ddcl.Text + "' and bisDeleted='False') and bisDeleted='False')";
            ...
            {
                string stnum = ddcl.SelectedItem.Value + ddsec.SelectedItem.Value + "-" + "1";
                txtstnum.Text = stnum;
            }
            }
            catch (Exception ex)
        ...
        public void concession_student()
        {
            ...
            if (dr.Read())
            {
                ...
                cmd.CommandText = "insert into tbl_ConcessionStudent(nStd_id,nConc_id,nc_id,nsc_id,userid,nsch_id,dtAddDate,bisDeleted)values((select ne_id from tbl_Enrollment where bRegisteredNum='" + txtadmnumber.Text + "'),(select nConc_id from tbl_Concession where strConcTitle='" + txtdisc.Text + "' and bisDeleted='False'),('" + ddcl.SelectedItem.Value+ "'),('" + ddsec.SelectedItem.Value+ "'),@uid,@schid,convert(date,sysdatetime()),'False')";
            ...
            else
            {
                ...
                cmd.CommandText = "insert into tbl_ConcessionStudent(nStd_id,nConc_id,nc_id,nsc_id,userid,nsch_id,dtAddDate,bisDeleted)values((select ne_id from tbl_Enrollment where bRegisteredNum='" + txtadmnumber.Text + "'),(select max(nConc_id) from tbl_Concession),('" + ddcl.SelectedItem.Value + "'),('" + ddsec.SelectedItem.Value+ "'),@uid,@schid,convert(date,sysdatetime()),'False')";
                            ...
                            else
                            {
                                ...
                                else
                                {
                                            ...
                                            //    con.Close();
                                               ...
                                               // dr = cmd.ExecuteReader(); 
                                                ...
                                                //else
                                                {
                                                    ...
                                                    cmd.Connection = con;
                                                    cmd.CommandType = CommandType.Text;
                                                    //cmd.CommandText = "insert into tbl_Enrollment(strAdmissionNumber,strStatus,bRegisteredNum,userid,strNIC,dtVerifyDate,nu_id,nfee_id,dtEn_Date,strFname,strLname,nc_id,nsc_id,strShift,strDOB,strBirthPlace,strNationality,strMotherlang,strReligion,strGender,strPhAddress,strPrAddress,strCity,strState,strCountry,nPinCode,strPhone,strMobile,strEmail,strPassword,strImage,nsch_id,strStudentNum,dtAddDate,bisDeleted) values (@admnum,'Confirm','" + regnumber + "','" + Session["uid"] + "',@stnic,CONVERT(VARCHAR(10), GETDATE(), 105 ),(select max(nu_id) from tbl_Users where nlevel=3 and bisDeleted=0 and nsch_id='" + Session["nschoolid"] + "'),(select nfee_id from tbl_Fee where nc_id=(select nc_id from tbl_Class where strClass='" + ddcl.Text + "' and bisDeleted=0 and nsch_id='" + Session["nschoolid"] + "')and bisDeleted=0 and nsch_id='" + Session["nschoolid"] + "'),CONVERT(VARCHAR(10), GETDATE(), 105 ),@fn,@ln,(select nc_id from tbl_Class where strClass=@clnm and bisDeleted=0 and nsch_id='" + Session["nschoolid"] + "'),(select nsc_id from tbl_Section where strSection='" + lblSection.Text + "' and nc_id=(select nc_id from tbl_Class where strClass='" + ddcl.Text + "' and bisDeleted=0 and nsch_id='" + Session["nschoolid"] + "')and bisDeleted=0 and nsch_id='" + Session["nschoolid"] + "'),@shft,@dob,@bplace,@nationality,@mlng,@relg,@gen,@phadrs,@pradrs,@city,@st,@cntry,@pin,@phn,@emphn,@em,@psd,@img,'" + Session["nschoolid"] + "','" + txtstnum.Text + "','" + txtadmsndate.Text + "','False')";
                                                    cmd.CommandText = "insert into tbl_Enrollment(strAdmissionNumber,strStatus,bRegisteredNum,userid,strNIC,dtVerifyDate,nu_id,nfee_id,dtEn_Date,strFname,strLname,nc_id,nsc_id,strShift,strDOB,strBirthPlace,strNationality,strMotherlang,strReligion,strGender,strPhAddress,strPrAddress,strCity,strState,strCountry,nPinCode,strPhone,strMobile,strEmail,strPassword,strImage,nsch_id,strStudentNum,dtAddDate,bisDeleted) values (@admnum,'Confirm','" + txtadmnumber.Text + "','" + Session["uid"] + "',@stnic,CONVERT(VARCHAR(10), GETDATE(), 105 ),(select nu_id from tbl_Users where strNIC='" + txtnic.Text+ "' and nlevel=3 and bisDeleted=0 and nsch_id='" + Session["nschoolid"] + "'),(select nfee_id from tbl_Fee where nc_id=('"+ddcl.SelectedItem.Value+"')and bisDeleted=0 and nsch_id='" + Session["nschoolid"] + "'),CONVERT(VARCHAR(10), GETDATE(), 105 ),@fn,@ln,('"+ddcl.SelectedItem.Value+"'),('"+ddsec.SelectedItem.Value+"'),@shft,@dob,@bplace,@nationality,@mlng,@relg,@gen,@phadrs,@pradrs,@city,@st,@cntry,@pin,@phn,@emphn,@em,@psd,@img,'" + Session["nschoolid"] + "','" + txtstnum.Text + "','" + txtadmsndate.Text + "','False')";
                                                    ...
                                                    ScriptManager.RegisterStartupScript(this, GetType(), "showalert", "alert('Your Child has been enrolled.'); window.location = 'AdminManageP-C.aspx';", true);
                                                }
                                            //}
                                        }
                                        else
                                    ...
                                    else
                                    {
                                                ...
                                                else
                                                {
                                                    ...
                                                    //    con.Close();
                                                        ...
                                                        //else
                                                        {
                                                            ...
                                                            Int64 regnumber = Convert.ToInt64(regnum);
                                                            regnumber += 1;
                                                            con.Close();
                                                            con.Open();
                                                            cmd.Connection = con;
                                                            ...
                                                            //
                                                            //cmd.Parameters.AddWithValue("@mn", txtml.Text);
                                                            cmd.CommandText = "insert into tbl_Enrollment(strAdmissionNumber,strStatus,bRegisteredNum,userid,strNIC,dtVerifyDate,nu_id,nfee_id,dtEn_Date,strFname,strLname,nc_id,nsc_id,strShift,strDOB,strBirthPlace,strNationality,strMotherlang,strReligion,strGender,strPhAddress,strPrAddress,strCity,strState,strCountry,nPinCode,strPhone,strMobile,strEmail,strPassword,strImage,nsch_id,strStudentNum,dtAddDate,bisDeleted) values (@admnum,'Confirm','" + txtadmnumber.Text + "','" + Session["uid"] + "',@stnic,CONVERT(VARCHAR(10), GETDATE(), 105 ),(select max(nu_id) from tbl_Users where nlevel=3 and bisDeleted=0 and nsch_id='" + Session["nschoolid"] + "'),(select nfee_id from tbl_Fee where nc_id=('"+ddcl.SelectedItem.Value+"')and bisDeleted=0 and nsch_id='" + Session["nschoolid"] + "'),CONVERT(VARCHAR(10), GETDATE(), 105 ),@fn,@ln,('"+ddcl.SelectedItem.Value+"'),('"+ddsec.SelectedItem.Value+"'),@shft,@dob,@bplace,@nationality,@mlng,@relg,@gen,@phadrs,@pradrs,@city,@st,@cntry,@pin,@phn,@emphn,@em,@psd,@img,'" + Session["nschoolid"] + "','" + txtstnum.Text + "','" + txtadmsndate.Text + "','False')";
                                                            cmd.Parameters.AddWithValue("@stnic", txtstnic.Text);
                                                            cmd.Parameters.AddWithValue("@admnum",txtadmnumber.Text);
                                                            cmd.Parameters.AddWithValue("@fn", txtsfn.Text);
                                                            cmd.Parameters.AddWithValue("@ln", txtsln.Text);
                                                            ...
                                                            // txtnic.Visible = true;
                                                            ScriptManager.RegisterStartupScript(this, GetType(), "showalert", "alert('Parent and Child  has been Registered.'); window.location = 'AdminManageP-C.aspx';", true);
                                                            // Response.Redirect("AdminManageP-C.aspx");
                                                            // Response.Redirect(Request.RawUrl);
                                                        }
                                                    //}
                                        ...
                                        {
                                            Response.Redirect("Error.aspx?msg=AdminManageP-C.aspx");
                                        }
                                    }
                                }
                            }
        ...
        protected void ddsec_SelectedIndexChanged(object sender, EventArgs e)
        {
          ...
          // ddsecValue =ddsec.SelectedValue;
            lblSection.Text = ddsec.SelectedItem.Text;
            //lblSectionn.Visible = true;
            SelectedSec.Visible = true;
            SelectedSec.Text = ddsec.SelectedItem.Text;
            //ddsec.Items.Clear();
            //ddsec.Items.Add("Select Section");
        }
