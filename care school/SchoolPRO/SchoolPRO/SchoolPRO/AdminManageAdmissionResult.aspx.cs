...
namespace SchoolPRO
{
    public partial class AdminManageAdmissionResult : System.Web.UI.Page
    {
        ...
        private DataTable GetRecordsOfStudents()
        {
            ...
            if (ddlType.SelectedIndex !=0)
            {
                cmd.CommandText = "SELECT ne_id, nc_id, nsc_id, nsch_id, bRegisteredNum as strStudentNum, dtAddDate FROM  tbl_Enrollment WHERE (bisDeleted = 1) AND (nsch_id ='" + Session["nschoolid"] + "')  AND (strStatus ='Pending') and nsc_id is null";
            }
            //else
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow re in dt.Rows)
                    {
                        ...
                        string scid = "0";//ddsec.SelectedValue; //re["nsc_id"].ToString();
        ...
        private DataTable GetRecordsOfSingleStudents(string neid, string cid, string scid)
        {
            ...
            if (ddcl.SelectedIndex != 0  && ddlType.SelectedIndex != 0)
            {
                try
                {
                    con.Open();
                    ...
                    {
                        cmd.CommandText = "SELECT tbl_Fee.nfee_id,tbl_Result.strTotalMarks,tbl_Enrollment.strFname+' '+tbl_Enrollment.strLname as Name , tbl_Enrollment.bRegisteredNum as strStudentNum, tbl_Subject.strSubject, tbl_Result.strType, tbl_Class.strClass ,tbl_Class.nc_id,tbl_Class.strClass as  strSection,tbl_Class.nc_id  as nsc_id, CAST(tbl_Result.strObtMarks AS float) / CAST(tbl_Result.strTotalMarks AS float) * 100 AS SubjectPer, CAST(CASE WHEN CAST(tbl_Result.strObtMarks AS float) / CAST(tbl_Result.strTotalMarks AS float) * 100 >= CAST(tbl_Percentage.strPercentage AS float) THEN 1 ELSE 0 END AS bit) AS Status, tbl_Percentage.strPercentage FROM tbl_Result INNER JOIN tbl_Enrollment ON tbl_Result.ne_id = tbl_Enrollment.ne_id INNER JOIN tbl_Subject ON tbl_Result.nsbj_id = tbl_Subject.nsbj_id INNER JOIN tbl_Class ON tbl_Enrollment.nc_id = tbl_Class.nc_id  CROSS JOIN tbl_Percentage INNER JOIN tbl_Fee ON tbl_Enrollment.nfee_id = tbl_Fee.nfee_id WHERE (tbl_Result.nAdmsn_id = '" + ddlType.SelectedValue + "') AND (tbl_Result.bisDeleted = 0)   AND (tbl_Result.nc_id='" + cid + "')  AND (tbl_Enrollment.nc_id = '" + cid + "') AND (tbl_Enrollment.ne_id = '" + neid + "') and tbl_Result.nsch_id= '" + Session["nschoolid"] + "' AND (tbl_Enrollment.bisDeleted = 'True') AND (tbl_Enrollment.strStatus = 'Pending') and tbl_Percentage.nsch_id='"+Session["nschoolid"]+"' ";
                    }
                    //else
        ...
        public void ShowStudentResult(string neid, string clas, string sec)
        {
            ...
            try
            {
                ...
                cmd.CommandType = CommandType.Text;
                List<StudentResult> data = new List<StudentResult>();
                cmd.CommandText = "SELECT tbl_Result.strTotalMarks,tbl_Result.strObtMarks, tbl_Enrollment.bRegisteredNum as strStudentNum,tbl_Enrollment.strFname+' '+tbl_Enrollment.strLname as Name ,tbl_Subject.strSubject,tbl_Subject.strCourseCode, tbl_Result.strType, tbl_Class.strClass,CAST(tbl_Result.strObtMarks AS float) / CAST(tbl_Result.strTotalMarks AS float) * 100 AS SubjectPer, CAST(CASE WHEN CAST(tbl_Result.strObtMarks AS float) / CAST(tbl_Result.strTotalMarks AS float) * 100 >= CAST(tbl_Percentage.strPercentage AS float) THEN 1 ELSE 0 END AS bit) AS Status, tbl_Percentage.strPercentage FROM  tbl_Result INNER JOIN tbl_Enrollment ON tbl_Result.ne_id = tbl_Enrollment.ne_id INNER JOIN   tbl_Subject ON tbl_Result.nsbj_id = tbl_Subject.nsbj_id INNER JOIN  tbl_Class ON tbl_Enrollment.nc_id = tbl_Class.nc_id  CROSS JOIN  tbl_Percentage WHERE (tbl_Result.nAdmsn_id = '" + ddlType.SelectedValue + "') AND (tbl_Result.bisDeleted = 0) AND (SUBSTRING(tbl_Result.dtAddDate ,7,10) = SUBSTRING(CONVERT(VARCHAR(10), GETDATE(), 105 ) ,7,10)) AND (tbl_Result.nc_id=@cid) AND (tbl_Enrollment.nc_id = @cid)  AND (tbl_Enrollment.ne_id = @neid) AND (tbl_Result.nsch_id = @schid) and tbl_Percentage.nsch_id=@schid AND (tbl_Enrollment.bisDeleted = 1) AND (tbl_Enrollment.strStatus = 'Pending')";
        ...
        protected void btnPromote_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (ddlType.SelectedIndex != 0)
                {
                    ...
                    foreach (GridViewRow gvr in gv_detail_list.Rows)
                    {
                        ...
                        if (status == "Pass")
                        {
                            ...
                            if (InsertFlag)
                            {
                                ...
                                cmd.Connection = con;
                                cmd.CommandType = CommandType.Text;
                                cmd.CommandText = "Update tbl_Enrollment set nc_id='" + ddclass1.SelectedValue + "',nsc_id='" + ddsec1.SelectedValue + "',nfee_id='" + newfeeid + "',nsch_id='" + Session["nschoolid"] + "' where ne_id='" + neid + "' and bisDeleted=1 and strStatus='Pending'";
                                cmd.ExecuteNonQuery();
                                con.Close();


                                ScriptManager.RegisterStartupScript(this, GetType(), "showalert", "alert('Students Admission is Now in Process...');", true);
                    ...
                    ScriptManager.RegisterStartupScript(this, GetType(), "showalert", "alert('Select Admission Marks...');", true);
            ...
            if (InsertFlag)
                Response.Redirect("AdminReportConfirmAdmissionSTD.aspx");
        ...
        public List<Boolean> checkresult()
        {
            Boolean flagofresultAnnc = false;
            Boolean flagofresultAnncDatePass = false;
            Boolean flagofresultAnncDateComng = false;
            ...
            try
            {
                ...
                cmd.CommandText = "select nExam_id,strStartDate st ,strEndDate ed from tbl_ResultShow where nExam_id=@eid and bisDeleted=@fbit and nsch_id=@schid";
                cmd.Parameters.Add("@fbit", BIT_T_F.FalseBit());
                cmd.Parameters.Add("@eid", ddlType.SelectedValue);
                ...
                {
                    dr.Read();
                    Session["stdt"] = dr["st"].ToString();
                    Session["eddt"] = dr["ed"].ToString();

                    string stdt = dr["st"].ToString();
                    string eddt = dr["ed"].ToString();

                    DateTime startdate = DateTime.ParseExact(stdt, "dd-MM-yyyy", null);
                    DateTime enddate = DateTime.ParseExact(eddt, "dd-MM-yyyy", null);
                    ...
                    if (startdate <= DateTime.Now)
                    {
                        if (enddate >= DateTime.Now)
                        {
                            ...
                            flagofresultAnnc = true;
                        }
                        else
                        {
                            flagofresultAnncDatePass = true;
                        }
                    ...
                    {
                        flagofresultAnncDateComng = true;
            ...
            flags.Add(flagofresultAnnc);
            flags.Add(flagofresultAnncDatePass);
            flags.Add(flagofresultAnncDateComng);

            return flags;
        }
        protected void ddlType_SelectedIndexChanged(object sender, EventArgs e)
        {
            //List<Boolean> flaglst = new List<bool>();
            //flaglst = checkresult();
            //if (!flaglst[0] && !flaglst[1] && !flaglst[2])
            //{
            //    Response.Redirect("ResultNotAnnonced.aspx?msg=Result Date Not Given&id=1");
            //}
            //else if (flaglst[1])
            //{
            //    Response.Redirect("ResultNotAnnonced.aspx?msg=Result Date Passed&id=2");
            //}
            //else if (flaglst[2])
            //{
            //    Response.Redirect("ResultNotAnnonced.aspx?msg=Result Date Comming&id=3");
            //}
