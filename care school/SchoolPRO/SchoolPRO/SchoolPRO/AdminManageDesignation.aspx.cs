...
namespace SchoolPRO
{
    public partial class AdminManageDesignation : System.Web.UI.Page
    {
        ...
        private DataTable GetRecords()
        {
            ...
            cmd.CommandText = "Select nDesg_id,strDesgName,strDesgCode from tbl_Designation where bisDeleted='False' and nsch_id='" + Session["nschoolid"] + "'";
        ...
        private void PopulateData()
        {
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["SchoolPro"].ToString()))
                {
                    ...
                    string sql = "Select nDesg_id,strDesgName,strDesgCode from tbl_Designation where bisDeleted='False' and nsch_id='" + Session["nschoolid"] + "'";
        ...
        private void SearchText(string strSearchText)
        {
            try
            {
                ...
                dv.RowFilter = "strDesgName like" + SearchExpression;
        ...
        protected void btnaddclass_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                cmd.Connection = con;
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "select nDesg_id from tbl_Designation where strDesgCode='" + txtDesigCode.Text + "' and bisDeleted='False' and nsch_id='" + Session["nschoolid"] + "'";
                ...
                else
                {
                    ...
                    cmd.Connection = con;
                    cmd.CommandType = CommandType.Text;
                    cmd.CommandText = "insert into tbl_Designation(strDesgName,nsch_id,nu_id,strDesgCode,dtAddDate,bisDeleted) values(@cname,'" + Session["nschoolid"] + "','" + Session["uid"] + "',@nos,CONVERT(VARCHAR(10), GETDATE(), 105 ),'False')";
                    cmd.Parameters.AddWithValue("@cname", txtaddDesig.Text);
                    cmd.Parameters.AddWithValue("@nos", txtDesigCode.Text);
                    cmd.ExecuteNonQuery();
                    con.Close();
                    txtaddDesig.Text = "";
                    ScriptManager.RegisterStartupScript(this, GetType(), "showalert", "alert('Designation has been registered successfully.'); ", true);
                   PopulateData();
        ...
        protected void btnupdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                cmd.Connection = con;
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "update tbl_Designation set strDesgName=@cname,dtAddDate= CONVERT(VARCHAR(10), GETDATE(), 105 ) where nDesg_id='" + Session["cid"] + "'";
        ...
        protected void btndel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                cmd.Connection = con;
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "update tbl_Designation set bisDeleted='True',dtDeleteDate=CONVERT(VARCHAR(10), GETDATE(), 105 ) where nDesg_id='" + id + "'";
                cmd.ExecuteNonQuery();
                con.Close();
                PopulateData();
                MultiView1.ActiveViewIndex = 0;
