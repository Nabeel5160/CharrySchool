...
namespace SchoolPRO
{
    public partial class AdminManageEmployeeSalary : System.Web.UI.Page
    {
        ...
        private DataTable GetRecords()
        {
            ...
            cmd.Connection = con;
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "Select s.nsal_id,u.strlname+' '+u.strfname as name,s.strSalary,s.strBonus,s.dtAddDate from tbl_Salary s inner join tbl_Users u on s.nu_id=u.nu_id where u.bisDeleted='False' and u.nsch_id=@schid and s.bisDeleted='False' and s.nsch_id=@schid   and (u.nLevel =@am_emp OR u.nLevel = @adm_emp OR u.nLevel = @lib_emp OR u.nLevel= @hstl_emp )";
        ...
        private void PopulateData()
        {
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["SchoolPro"].ToString()))
                {
                    ...
                    string sql = "Select s.nsal_id,u.strlname+' '+u.strfname as name,s.strSalary,s.strBonus,s.dtAddDate from tbl_Salary s inner join tbl_Users u on s.nu_id=u.nu_id where u.bisDeleted='False' and u.nsch_id=@schid and s.bisDeleted='False' and s.nsch_id=@schid   and (u.nLevel =@am_emp OR u.nLevel = @adm_emp OR u.nLevel = @lib_emp OR u.nLevel= @hstl_emp )";
        ...
        public void status()
        {
            if (ddenm.SelectedItem.ToString() != "--Select Employee--")
            {
                try
                {
                    ...
                    cmd.Connection = con1;
                    cmd.CommandType = CommandType.Text;
                    //cmd.CommandText = "select count(strStatus) from tbl_EmployeeAttendance where strStatus='Present' and nu_id=(select nu_id from tbl_Users where strlname='" + ddenm.Text + "' and nLevel=2 or nLevel=4)";
                    cmd.CommandText = "select count(strStatus) from tbl_EmployeeAttendance where strStatus='Present' and nu_id='" + Convert.ToInt32(ddenm.Text) + "' and SUBSTRING(dtAddDate ,4,2)=SUBSTRING(CONVERT(VARCHAR(10), GETDATE(), 105 ) ,4,2) and SUBSTRING(dtAddDate ,7,10)=SUBSTRING(CONVERT(VARCHAR(10), GETDATE(), 105 ) ,7,10) and bisDeleted='False' and nsch_id=@schid";
                    cmd.Parameters.AddWithValue("@schid", Session["nschoolid"]);
                    ...
                    cmd.Connection = con1;
                    cmd.CommandType = CommandType.Text;
                    //cmd.CommandText = "select count(strStatus) from tbl_EmployeeAttendance where strStatus='Abscent' and nu_id=(select nu_id from tbl_Users where strlname='" + ddenm.Text + "' and nLevel=2 or nLevel=4)";
                    cmd.CommandText = "select count(strStatus) from tbl_EmployeeAttendance where strStatus='Absent' and nu_id='" + Convert.ToInt32(ddenm.SelectedValue) + "' and SUBSTRING(dtAddDate ,4,2)=SUBSTRING(CONVERT(VARCHAR(10), GETDATE(), 105 ) ,4,2) and SUBSTRING(dtAddDate ,7,10)=SUBSTRING(CONVERT(VARCHAR(10), GETDATE(), 105 ) ,7,10) and bisDeleted='False' and nsch_id=@schid1";
                    ...
                    cmd.Connection = con1;
                    cmd.CommandType = CommandType.Text;
                    //cmd.CommandText = "select count(strStatus) from tbl_EmployeeAttendance where strStatus='Half Leave Application' OR strStatus='Full Leave Application' and nu_id=(select nu_id from tbl_Users where strlname='" + ddenm.Text + "' and nLevel=2 or nLevel=4)";
                    cmd.CommandText = "select count(strStatus) from tbl_EmployeeAttendance where nu_id='" + Convert.ToInt32(ddenm.SelectedValue) + "' and strStatus='Half Leave Application' and SUBSTRING(dtAddDate ,4,2)=SUBSTRING(CONVERT(VARCHAR(10), GETDATE(), 105 ) ,4,2) and SUBSTRING(dtAddDate ,7,10)=SUBSTRING(CONVERT(VARCHAR(10), GETDATE(), 105 ) ,7,10) and bisDeleted='False' and nsch_id=@schid2";
                    ...
                    cmd.CommandText = "select strSalary from tbl_Users where nu_id='" + Convert.ToInt32(ddenm.SelectedValue) + "'and bisDeleted='False' and nsch_id=@schid5";
