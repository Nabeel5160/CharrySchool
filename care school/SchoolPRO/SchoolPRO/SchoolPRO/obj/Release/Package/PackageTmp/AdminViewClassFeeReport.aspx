<%@ Page Title="" Language="C#" MasterPageFile="~/Admin.Master" AutoEventWireup="true" CodeBehind="AdminViewClassFeeReport.aspx.cs" Inherits="SchoolPRO.AdminViewClassFeeReport" %>
                                        ...
                                        <div class="table-responsive">
                                            ...
                                            <asp:GridView ID="gv_detail_list" CssClass=" table table-striped table-bordered table-hover" runat="server" AutoGenerateColumns="False" DataSourceID="fEEREPORT">
                                                <Columns>
                                                    ...
                                                    <asp:BoundField DataField="Reg #" HeaderText="Reg #" ReadOnly="True" SortExpression="Reg #"></asp:BoundField>
                                                    ...
                                                    <asp:BoundField DataField="ConcessionPer" HeaderText="Concession Percentage" SortExpression="Concession Percentage" ReadOnly="True"></asp:BoundField>
                                            ...
                                            <asp:SqlDataSource runat="server" ID="fEEREPORT" ConnectionString='<%$ ConnectionStrings:SchoolPro %>' SelectCommand="SELECT tbl_RecieveFee.nChallanNum AS 'Challan #', CAST(tbl_Enrollment.bRegisteredNum AS varchar(MAX)) AS 'Reg #', tbl_Enrollment.strFname + ' ' + tbl_Enrollment.strLname AS 'Student Name', tbl_Users.strfname + ' ' + tbl_Users.strlname AS 'Guardian Name', tbl_Users.strPhone AS 'Guardian  Phone', tbl_Class.strClass AS Class, tbl_Section.strSection AS Section, tbl_Fee.strTutionFee AS 'Class Fee', tbl_RecieveFee.strFeeAmount AS 'Class Fee Charged', CASE WHEN tbl_RecieveFee.strFeeConcession = 0 THEN 'Nill' ELSE CAST((((CAST(tbl_Fee.strTutionFee AS float) - CAST(tbl_RecieveFee.strFeeAmount AS float))) / CAST(tbl_Fee.strTutionFee AS float)) * 100 AS varchar(MAX)) + '%' END AS ConcessionPer, tbl_RecieveFee.strFeeAmountReceived AS 'Recieved Fee', tbl_RecieveFee.strFeeAmountRemaining AS 'Remaining Fee', tbl_RecieveFee.strFeeConcession AS 'Concession Fee', tbl_RecieveFee.strFeeMonth AS 'Fee Month', tbl_RecieveFee.strMonths AS 'Months Fee Paid', tbl_RecieveFee.dtAddDate AS 'Recieve Date' FROM tbl_RecieveFee INNER JOIN (SELECT nc_id, strClass FROM tbl_Class AS tbl_Class_1 WHERE (bisDeleted = 'False') AND (nsch_id = @schid)) AS tbl_Class ON tbl_RecieveFee.nc_id = tbl_Class.nc_id INNER JOIN (SELECT nsc_id, strSection FROM tbl_Section AS sec WHERE (bisDeleted = 'False') AND (nsch_id = @schid)) AS tbl_Section ON tbl_RecieveFee.nsc_id = tbl_Section.nsc_id INNER JOIN (SELECT nsc_id, bRegisteredNum, strStudentNum, strFname, strLname, ne_id, nc_id, nfee_id, nu_id FROM tbl_Enrollment AS enroll WHERE (bisDeleted = 'False') AND (nsch_id = @schid)) AS tbl_Enrollment ON tbl_RecieveFee.ne_id = tbl_Enrollment.ne_id INNER JOIN (SELECT strfname, strlname, nu_id, strPhone FROM tbl_Users AS usr WHERE (bisDeleted = 'False') AND (nsch_id = @schid)) AS tbl_Users ON tbl_Enrollment.nu_id = tbl_Users.nu_id INNER JOIN (SELECT nfee_id, strTutionFee FROM tbl_Fee AS fe WHERE (bisDeleted = 'False') AND (nsch_id = @schid)) AS tbl_Fee ON tbl_Enrollment.nfee_id = tbl_Fee.nfee_id WHERE (tbl_RecieveFee.bisDeleted = 'False') AND (tbl_RecieveFee.nsch_id = @schid) AND (SUBSTRING(tbl_RecieveFee.strFeeMonth, 4, 2) = @month) AND (tbl_RecieveFee.strRecieveType = 'Class') AND (tbl_RecieveFee.nc_id = @cid) AND (tbl_RecieveFee.nsc_id = @sc) AND (SUBSTRING(tbl_RecieveFee.strFeeMonth, 7, 10) = SUBSTRING(CONVERT (VARCHAR(10), GETDATE(), 105), 7, 10))">
                                                <SelectParameters>
