...
namespace SchoolPRO
{
    public partial class TeacherAddExamMarks : System.Web.UI.Page
    {
                            ...
                            if (marks != "" && txttmrks.Text != "")
                            {
                                if (Convert.ToInt32(marks) <= Convert.ToInt32(txttmrks.Text))
                                {
                                    if (row.Enabled == true)
                                    {
                                        ...
                                        cmd.Connection = con;
                                        cmd.CommandType = CommandType.Text;
                                        cmd.CommandText = "insert into tbl_Result(nsch_id,nc_id,nsc_id,nsbj_id,ne_id,nu_id,strTotalMarks,strObtMarks,strRemarks,strType,nExam_id,dtAddDate,bisDeleted) values('" + Session["nschoolid"] + "','" + Session["cname"] + "','" + Session["section"] + "','" + Session["courscode"] + "','" + student_num + "',(select nu_id from tbl_Users where strEmail='" + Session["userval"] + "' and bisDeleted='False' and nsch_id='" + Session["nschoolid"] + "'),@obt,@st,@rem,@examname,@examtypeid,CONVERT(VARCHAR(10), GETDATE(), 105 ),'False')";
                                        cmd.Parameters.AddWithValue("@st", marks);
                                        cmd.Parameters.AddWithValue("@obt", txttmrks.Text);
                                        cmd.Parameters.AddWithValue("@rem", remarks);
        ...
        private void PopulateData(string cls, string sec, string sub)
        {
            ...
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["SchoolPro"].ToString()))
            {
                ...
                string sql = "SELECT ex.nr_id, e.strStudentNum, e.strFname, e.strLname,ex.strTotalMarks, ex.strObtMarks, ex.strRemarks FROM tbl_Enrollment AS e INNER JOIN tbl_Result AS ex ON e.ne_id = ex.ne_id WHERE (ex.nc_id = @cname) AND (ex.nsbj_id = @cc) and (ex.nsc_id=@sce111) and ex.nu_id=@uem AND (ex.bisDeleted = 'False') and ex.nsch_id=@schid2 and ex.nQuiz_id is Null";
            ...
            foreach (GridViewRow row in GridView2.Rows)
            {
                if (row.RowType == DataControlRowType.DataRow)
                {
                    if (ddexams.SelectedIndex != 0)
                    {
                        ...
                        //}
                        //else
                            cmd.CommandText = "Select nr_id,strObtMarks,strRemarks,strTotalMarks from tbl_Result Where nExam_id='" + ddexams.SelectedValue + "' and ne_id='" + id + "' AND SUBSTRING(dtAddDate ,7,10)=SUBSTRING(CONVERT(VARCHAR(10), GETDATE(), 105 ) ,7,10) and nsbj_id='" + Session["courscode"] + "'  and bisDeleted=0 and nsch_id='" + Session["nschoolid"] + "' ";
