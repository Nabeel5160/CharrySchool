<%@ Page Title="" Language="C#" MasterPageFile="~/Teacher.Master" AutoEventWireup="true" CodeBehind="TeacherNewAdmissionMarks.aspx.cs" Inherits="SchoolPRO.TeacherNewAdmissionMarks" %>
                ...
                <asp:MultiView ID="mvmarks" ActiveViewIndex="0" runat="server">
                    <asp:View ID="View1" runat="server">
                        ...
                        <div class="table-responsive">
                            ...
                            <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:SchoolPro %>" SelectCommand="SELECT c.strClass, c.nc_id, sc.strSection, sc.nsc_id, s.strCourseCode, s.strSubject, s.nsbj_id FROM tbl_TeacherSubject AS t INNER JOIN tbl_Class AS c ON t.nc_id = c.nc_id INNER JOIN tbl_Subject AS s ON t.nsbj_id = s.nsbj_id INNER JOIN tbl_Users AS u ON t.nu_id = u.nu_id INNER JOIN tbl_Section as sc ON c.nc_id = sc.nc_id WHERE (u.strEmail = @uem) AND (t.bisDeleted = 'False') AND (t.nsch_id = @schid)">
                                <SelectParameters>
                    ...
                    <asp:View ID="View2" runat="server">
                        <div class="table-responsive">
                            ...
                            <asp:GridView ID="GridView2" class="table table-striped table-bordered table-hover" runat="server" AllowPaging="True" AutoGenerateColumns="False" DataSourceID="SqlDataSource2">
                                <Columns>
                            ...
                            <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:SchoolPro %>" SelectCommand="SELECT ne_id,strStudentNum, strFname, strLname FROM tbl_Enrollment WHERE (nc_id =  @cnm)  AND (strStudentNum IS NOT NULL) and nsch_id=@sn and bisDeleted='True' and strStatus='Pending'">
                                <SelectParameters>
