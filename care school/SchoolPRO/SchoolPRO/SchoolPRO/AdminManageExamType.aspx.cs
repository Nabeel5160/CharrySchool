...
namespace SchoolPRO
{
    public partial class AdminManageExamType : System.Web.UI.Page
    {
        ...
        private DataTable GetRecords()
        {
            ...
            //
            cmd.CommandText = "Select nExam_id,strExamName,strExamCode from tbl_ExamType where bisDeleted='False' and nsch_id='" + Session["nschoolid"] + "'";
        ...
        protected void btnupdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                cmd.Connection = con;
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "update tbl_ExamType set strExamName=@enos,dtUpDate= CONVERT(VARCHAR(10), GETDATE(), 105 ),nUpDateBy='"+Session["uid"]+"' where nExam_id='" + Session["cid"] + "'";
        ...
        protected void btnaddclass_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                cmd.CommandText = "select nExam_id from tbl_ExamType where strExamCode='" + txtaddexamcode.Text + "' and bisDeleted='False'";
                ////CLASS NAME SHOULD BE UNIQUIE IN ALL SCHOOLS///////////      
                dr = cmd.ExecuteReader();
                if (dr.HasRows)
                {
                    ScriptManager.RegisterStartupScript(this, GetType(), "showalert", "alert('Exam Type already exist.');", true);
                }
                else
                {
                    con.Close();
                    ...
                    cmd.Connection = con;
                    cmd.CommandType = CommandType.Text;
                    cmd.CommandText = "insert into tbl_ExamType(strExamName,strExamCode,nsch_id,nu_id,dtAddDate,bisDeleted) values(@examname,@examcode,'" + Session["nschoolid"] + "','" + Session["uid"] + "',CONVERT(VARCHAR(10), GETDATE(), 105 ),'False')";
                    ...
                    con.Close();
                    txtaddexam.Text = "";
                    ScriptManager.RegisterStartupScript(this, GetType(), "showalert", "alert('Exam Type has been registered successfully.'); window.location = 'AdminManageExamType.aspx';", true);
                    //PopulateData();
                    //MultiView1.ActiveViewIndex = 0;
                }
            }
        ...
        protected void btndel_Click(object sender, EventArgs e)
        {
            try
            {
               ...
               string id = gvr.Cells[1].Text;
                ...
                cmd.Connection = con;
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "update tbl_ExamType set bisDeleted='True',dtDeleteDate=CONVERT(VARCHAR(10), GETDATE(), 105 ) where nExam_id='" + id + "'";
                cmd.ExecuteNonQuery();
                con.Close();
                PopulateData();
                MultiView1.ActiveViewIndex = 0;
